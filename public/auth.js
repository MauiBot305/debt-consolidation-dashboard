...
window.Auth = window.Auth || {
  ...
  showLogin() {
    ...
    if (typeof lucide !== 'undefined') {
      if (typeof lucide !== 'undefined' && lucide.createIcons) { try { if (typeof lucide !== 'undefined' && lucide.createIcons) { try { if (typeof lucide !== 'undefined' && lucide.createIcons) { try { lucide.createIcons(); } catch(e) { console.warn('[Lucide] createIcons failed:', e.message); } }; } catch(e) { console.warn('[Lucide] createIcons failed:', e.message); } }; } catch(e) { console.warn('[Lucide] createIcons failed:', e.message); } };
    }
  ...
  handleLogin(email, password) {
    ...
    if (result.success) {
      // Show success and render dashboard without reload
      ...
      setTimeout(() => {
        ...
        try {
          ...
          if (appRef && typeof appRef.init === 'function') {
            ...
            if (app) {
              app.innerHTML = '<div style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:#0a0f1a"><div style="text-align:center"><h2 style="color:#ef4444;font-size:20px;margin-bottom:12px">Dashboard failed to load</h2><p style="color:#94a3b8;margin-bottom:16px">App module not available. Please refresh the page.</p><button onclick="location.reload()" style="padding:10px 20px;background:#3b82f6;color:white;border:none;border-radius:8px;cursor:pointer">Refresh</button></div></div>';
            }
          }
        } catch (e) {
