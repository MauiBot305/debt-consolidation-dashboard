...
// Get button/input/select/checkbox/table/tab catalog in a single evaluate
...
// Click button by index, return what happened
async function clickButton(page, idx) {
  ...
  const clicked = await page.evaluate((i) => {
    ...
    if (i >= btns.length) return false;
    btns[i].click();
    return true;
  }, idx);

  if (!clicked) { page.removeListener('dialog', dh); return 'Could not click'; }
...
async function testPage(page, pi) {
  // Navigate and catalog
  await nav(page, pi.page);
  const cat = await catalog(page);
  if (!cat) return { catalog: { heading: 'ERROR: no main', buttons:[], inputs:[], selects:[], tables:[], checkboxes:[], tabs:[] }, buttonResults:[], inputResults:[], selectResults:[], checkboxResults:[] };
  ...
  for (let i = 0; i < cat.buttons.length; i++) {
    ...
    // Navigate fresh before each button click
    await nav(page, pi.page);

    const result = await clickButton(page, i);
    const working = result === 'No visible change' ? '⚠️' : (result.includes('Could not') || result.includes('Error') ? '❌' : '✅');
    ...
    return [...main.querySelectorAll('input:not([type=hidden]):not([type=checkbox]):not([type=radio]), textarea')]
      ...
      .map((inp, i) => {
        ...
        try {
          ...
          if (setter) setter.call(inp, 'QA_TEST');
          else inp.value = 'QA_TEST';
          inp.dispatchEvent(new Event('input', { bubbles: true }));
          const accepted = inp.value === 'QA_TEST';
          if (setter) setter.call(inp, orig); else inp.value = orig;
          inp.dispatchEvent(new Event('input', { bubbles: true }));
          return { idx: i+1, placeholder: inp.placeholder||'', type: inp.type||'text', id: inp.id||'', acceptsInput: accepted, disabled: inp.disabled };
        } catch(e) { return { idx: i+1, placeholder: inp.placeholder||'', type: inp.type||'text', acceptsInput: false, error: e.message }; }
      });
  });
...
function fmtReport(passes) {
  ...
  for (const pi of PAGES) {
    for (let p = 0; p < passes.length; p++) {
      ...
      for (const b of d.buttonResults)
        md += `| ${b.idx} | ${(b.text||'').replace(/\|/g,'\\|').substring(0,50)} | ${(b.result||'').replace(/\|/g,'\\|').replace(/\n/g,' ').substring(0,70)} | ${b.working} |\n`;
      md += `\n`;
      ...
      md += `### Tables (${c.tables?.length||0})\n`;
      if (c.tables?.length) for (const t of c.tables) md += `- Table ${t.idx}: [${t.headers.join(', ')}] — ${t.rowCount} rows\n`;
      else md += `- None\n`;
      md += `\n`;
      ...
      if (d.checkboxResults?.length) {
        md += `| # | ID | Label | Toggles? |\n|---|-----|-------|----------|\n`;
        for (const cb of d.checkboxResults) md += `| ${cb.idx} | ${cb.id||'(anon)'} | ${cb.label||'(none)'} | ${cb.toggled?'✅':'❌'} |\n`;
      } else md += `- None\n`;
      md += `\n`;

      if (c.tabs?.length) { md += `### Tabs\n`; for (const t of c.tabs) md += `- "${t.text}"\n`; md += `\n`; }
  ...
  md += `| Page | P1 Btns | P2 Btns | P3 Btns | P1 Inp | P2 Inp | P3 Inp | Consistent? |\n`;
  md += `|------|---------|---------|---------|--------|--------|--------|-------------|\n`;
  for (const pi of PAGES) {
